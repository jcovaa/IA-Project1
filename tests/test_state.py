...
class TestGameState(unittest.TestCase):
   ...
   # Test the game_states function
   def test_game_states_successors(self):
      successors = game_states(self.state)

      self.assertEqual(len(successors), 6)
      self.assertTrue(all(cost == 1 for _, cost in successors))

      actual_bottles = [next_state.bottles for next_state, _ in successors]
      actual_cost =[cost for _, cost in successors]

      expected_bottles = [
         [[1, 2, 1], [2, 2, 1, 1], [3, 3, 2, 3], [3], []],
         [[1, 2, 1], [2, 2, 1, 1], [3, 3, 2, 3], [], [3]],
         [[1, 2, 1, 3], [2, 2], [3, 3, 2, 3], [1, 1], []],
         [[1, 2, 1, 3], [2, 2], [3, 3, 2, 3], [], [1, 1]],
         [[1, 2, 1, 3], [2, 2, 1, 1], [3, 3, 2], [3], []],
         [[1, 2, 1, 3], [2, 2, 1, 1], [3, 3, 2], [], [3]],
      ]
      
      self.assertEqual(len(successors), 6)
      self.assertTrue(all(c == 1 for c in actual_cost))
      self.assertCountEqual(actual_bottles, expected_bottles)
